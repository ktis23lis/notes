...
public class NotesSourceImpl implements NotesSource{
    ...
    public NotesSource init(NameNotesResponse nameNotesResponse){
        String[]lists = resources.getStringArray(R.array.list);
        String[]text = resources.getStringArray(R.array.text_of_notes);
        for (int i =0; i < lists.length; i++){
            dataSource.add(new NameNotes(lists[i],text[i],Calendar.getInstance().
                    getTime().toString()));
        }
        if (nameNotesResponse != null){
            nameNotesResponse.initialized(this);
        }
        return this;
    }
